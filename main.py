...
fenetre = Tk()
canvas = Canvas(fenetre, width=310*4, height=167.75*4)
...
#White ball
balls.append(Ball([width/4, height/4], [0, 0], len(balls), canvas, radius, alpha, cw, cb, "white", dt))

#Other balls
if n == 2:
    color = ["yellow", "red"]
    positions = [[width / 4, 3 * height / 4], [3 * width / 4, height / 2]]
    for k in range (n):
        position = positions[k]
        vitesse = [0, 0]
        balls.append(Ball(position, vitesse, len(balls), canvas, radius, alpha, cw, cb, color[k], dt))
else:
    for k in range (n):
        position = [random.random () * width, random.random () * height]
        vitesse = [(random.random () - 0.5) * 20000, (random.random () - 0.5) * 20000]
        balls.append(Ball(position, vitesse, len(balls), canvas, radius, alpha, cw, cb, randomColor(), dt))
...
def draw_lines(event, billiard, canvas, canvas_lines):
    ...
    for i in range(25):
        if i < len(lines) - 1:
            ...
            canvas.coords(canvas_lines[i], 0, 0, 0, 0)
...
def main(player, billiard, dt):
    ...
    fenetre.after(1, lambda: main(player, billiard, dt))
